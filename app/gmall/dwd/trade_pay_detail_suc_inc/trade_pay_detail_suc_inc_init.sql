          ...
          WHERE type = 'bootstrap-insert'
            AND data.payment_status = '1602') pi
             LEFT JOIN (SELECT data.order_id,
                        ...
                        WHERE type = 'bootstrap-insert') od ON pi.order_id = od.order_id
             LEFT JOIN (SELECT data.id, data.province_id
                        FROM order_info_inc
                        WHERE type = 'bootstrap-insert') oi ON pi.order_id = oi.id
             LEFT JOIN (SELECT data.order_id,
                               data.activity_id,
                               data.activity_rule_id
                        FROM order_detail_activity_inc
                        WHERE type = 'bootstrap-insert') oda ON pi.order_id = oda.order_id
             LEFT JOIN (SELECT data.order_id, data.coupon_id
                        FROM order_detail_coupon_inc
                        WHERE type = 'bootstrap-insert') odc ON pi.order_id = odc.order_id
